...
def xtest_stress_test_no_preselection():
  ...
  error, changed_anything = select_greedy(SelectionDefaultParameters(dataset, OrderedSet(("base",)), "test"), GreedySelectionParameters(
...
def xtest_stress_test_with_preselection_and_subset():
  ...
  error, changed_anything = select_greedy(SelectionDefaultParameters(dataset, OrderedSet(("from",)), "to"), GreedySelectionParameters(
    lines, "|", True, SelectionMode.FIRST), WeightSelectionParameters(weights, 4, True, False), 10_000, 16, None, getLogger())
...
if __name__ == "__main__":
  ...
  xtest_stress_test_with_preselection()
  # test_stress_test_no_preselection()
